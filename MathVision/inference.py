...
pre_prompt = "Hint: Please answer the question requiring a floating-point number with one decimal place and provide the final value, e.g., 1.2, 1.3, 1.4, at the end.\n"
pre_prompt_w_choices = "Hint: Please answer the question and provide the correct option letter, e.g., A, B, C, D, at the end.\n"
...
def load_dataset(
    ...
    for idx, data in enumerate(tqdm.tqdm(raw_dataset)):
        ...
        if is_multiple_choice:
            problem = pre_prompt_w_choices + data["problem_w_choices"]
        else:
            problem = pre_prompt + data["problem"]
...
def main(args):
    ...
    print(f"开始处理MathVision数据集: {args.input_file}")
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="MathVision evaluation script with Qwen2.5-VL-7B-Instruct"
    )
