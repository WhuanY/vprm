...
def process(img_arg):
    ...
    try:
        while '</answer>' not in response_message:
            ...
            if response.choices[0].stop_reason == '</tool_call>':
                response_message += '</tool_call>'
