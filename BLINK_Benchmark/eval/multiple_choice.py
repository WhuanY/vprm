...
model = OpenAI(
    base_url=os.environ.get("CUSTOMIZED_REMOTE_OPENAI_API_ENDPOINT", ""),
    api_key=os.environ.get('CUSTOMIZED_REMOTE_OPENAI_API_KEY', "")
)
# client = OpenAI(
#   base_url=xiaojingai_url,
#   api_key=api_key,
# )
model_name = 'gpt-4o-mini'
print(model)
print(os.environ.get("CUSTOMIZED_REMOTE_OPENAI_API_ENDPOINT", ""))
print(os.environ.get('CUSTOMIZED_REMOTE_OPENAI_API_KEY', ""))
...
def match_multiple_choice(question, options, prediction):
    ...
    for retry in range(retry_limit):
        try:
            ...
            print(e, flush=True)
            time.sleep(1)
    print("[WARNING] match_multiple_choice failed after retries. This would affect the evaluation accuracy.")
