...
def single_record(record: dict, index: int):
    ...
    if 'image' in record and record['image']:
        # print("yes!!!")
        # print("type of image: ",  type(record['image'])) # -> type of image:  <class 'bytes'>
        image_filename = record['meta_info']['image_path'].split('/')[-1]
        print("image_filename:", image_filename)
        image_path = save_image_from_bytes(record['image'], image_filename)
        if image_path:
...
def main():
    ...
    for file_path in valid_files:
        ...
        for record in tqdm(processed_lsts, desc=f"Processing {os.path.basename(file_path)}"):
            print(record.keys())
            id2category[record['index']] = dict()
            id2category[record['index']]['category'] = record['category']
            id2category[record['index']]['l2_category'] = record['l2_category']
    ...
    with open("data/mmstar_id2category.json", 'w', encoding='utf-8') as f:
        json.dump(id2category, f, ensure_ascii=False)
    ...
    print(f"Index to category mapping saved to mmstar_id2category.json")
